...
def decode_UTF8String(data,dlen):
    logging.debug(" length of UTF %d --- %s", dlen, data )
    if dlen < 8:
	dlen = 9 
    fs="!"+str(dlen-8)+"s"
...
def decode_Grouped(data):
    ...
    for gmsg in splitMsgAVPs(data):
        ...
        #print 'Decoded now is ',decodeAVP(gmsg)
        #mylist.append(decodeAVP(gmsg))
    #print 'LIST',mylist
    return ret
...
def encode_UTF8String(A,flags,data):
    ...
    logging.debug("data for packing here is %s,%d",data, len(data));
...
def decodeAVP(msg):
    ...
    if mflags & DIAMETER_FLAG_VENDOR:
        ...
        logging.debug('message is ----%s, %s',msg, tmp_msg)
        data_len-=4
    A=AVPItem()
    ...
    if A.type in asUTF8:
        decoded=True
        logging.debug("Decoding UTF8String %d", data_len)
