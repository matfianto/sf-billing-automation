...
if len(sys.argv)-1 < 6:
  print 'Usage: SF-VOICE-Simulator.py <Session-Name> <MDN-A> <MDN-B> <MDN-C> <TIME> <DURATION>'
  print 'Input param "default" will set value Session-Name:default MDN-A:6288295291295 MDN-B:6288295291296 MDN-C:6288295291297 TIME:default-now DURATION:10'
  sys.exit(2)
SESSION=sys.argv[1]
A_NO=sys.argv[2]
B_NO=sys.argv[3]
...
def send_voice_init(session_id,CCRequestNumber,Timestamp,Anumber,Bnumber,CNumber):
 CCR_avps=[]
 ...
 CCR_avps.append(encodeAVP('Service-Information', [ 
    encodeAVP('IN-Information', [
      ...
      encodeAVP('Called-Party-Number', CNumber),
      ...
      encodeAVP('Redirecting-Party-Id', Bnumber),
      encodeAVP('Redirection-Information', '3'),
      encodeAVP('Original-Called-Party-Id', Bnumber)
    ])
 ]))
...
def send_voice_update(session_id,CCRequestNumber,Timestamp,Anumber,Bnumber,Cnumber,RAT_Type,RatingGroup,User_Equip,ReqUnit,UseUnit,CCTime,IMSI):
 CCR_avps = []
 CCR_avps.append(encodeAVP('Session-ID', session_id))
 CCR_avps.append(encodeAVP('Origin-Host', 'SCP001.chinatelecom.com'))
 CCR_avps.append(encodeAVP('Origin-Realm', 'chinatelecom.com'))
 ...
 CCR_avps.append(encodeAVP('Service-Information', [ 
    encodeAVP('IN-Information', [
      ...
      encodeAVP('Service-Key', 1),
      encodeAVP('Calling-Vlr-Number', '086149051'),
      encodeAVP('Calling-CellID-Or-SAI', '086149051'),
      encodeAVP('Calling-LAI', '086149051'),
      encodeAVP('Called-Vlr-Number', '0861390510'),
      encodeAVP('Called-CellID-Or-SAI', '0861390510'),
      encodeAVP('Called-LAI', '0861390510'),
...
def send_voice_terminate(session_id,CCRequestNumber,Timestamp,Anumber,Bnumber,Cnumber,CCTime):
 CCR_avps = []
 CCR_avps.append(encodeAVP('Session-ID', session_id))
 CCR_avps.append(encodeAVP('Origin-Host', 'SCP001.chinatelecom.com'))
 CCR_avps.append(encodeAVP('Origin-Realm', 'chinatelecom.com'))
...
if B_NO.lower() == 'default':
 B_NO = '6288295291296'
if C_NO.lower() == 'default':
 C_NO = '6288295291297'
if DURATION.lower() == 'default':
 DURATION = 10
print '-----------------------------------------------------------------------------------------------------------------------------------------------------------------'
print 'Execution at ',datetime.now()
print 'Sending VOICE Usage--> Session-Name:',session_id,' ## MDN A:',A_NO,' ## MDN B:',B_NO,' ## MDN C:',C_NO,' ## TIME:',datetime.fromtimestamp(Timestamp),' ## DURATION:',DURATION
#Voice Initial session_id,CCRequestNumber,Timestamp,Anumber,Bnumber,RatingGroup,CCTime,IMSI,ANI
result_code=send_voice_init(session_id,0,Timestamp,A_NO,B_NO,C_NO)
if result_code == 2001:
 ...
 result_code1=send_voice_terminate(session_id,1,Timestamp,A_NO,B_NO,C_NO,DURATION)
