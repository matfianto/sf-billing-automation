...
public class DBTablePrinter {
	 ...
	 */
	private static final int DEFAULT_MAX_TEXT_COL_WIDTH = 300;
	...
	public static void printResultSet(ResultSet rs, int maxStringColWidth) {
		...
		try {
			...
			/*			if (rs.last()) {
			 rowCount = rs.getRow();
			 KeywordUtil.logInfo("rowCount :"+rowCount)
			...
			tablePrint=tablePrint+info

			// Print out the formatted column labels
			tablePrint=tablePrint+strToPrint.toString();
			...
			for (int i = 0; i < rowCount; i++) {
				for (Column c : columns) {
					...
					tablePrint=tablePrint+String.format(format, c.getValue(i).toString())
				}

				tablePrint=tablePrint+"|\n"
				tablePrint=tablePrint+rowSeparator;
			}
