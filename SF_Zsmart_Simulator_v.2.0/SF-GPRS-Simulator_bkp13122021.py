...
if len(sys.argv)-1 < 7:
  print 'Usage: SF-GPRS-Simulator.py <Session-Name> <MDN> <RG> <TIME> <User-Equip> <Cell_ID> <UsageInput>'
  print 'Input param "default" will set value Session-Name:default MDN:6288295291295 TIME:default-now RG:4 User-Equip:867345 0103865801 Cell_ID:1527-38480663 UsageInput:341333'
  sys.exit(2)
SESSION=sys.argv[1]
MDNINP=sys.argv[2]
RGINP=sys.argv[3]
...
def send_gprs_init(session_id,CCRequestNumber,Timestamp,Anumber,RAT_Type,RatingGroup,User_Equip,Cell_ID):
 CCR_avps=[]
...
def send_gprs_update(session_id,CCRequestNumber,Timestamp,Anumber,RAT_Type,RatingGroup,User_Equip,Cell_ID,ReqUnit,UseUnit,CCTime):
 CCR_avps = []
 CCR_avps.append(encodeAVP('Session-ID', session_id))
 CCR_avps.append(encodeAVP('Origin-Host', 'SCP001.chinatelecom.com'))
 CCR_avps.append(encodeAVP('Origin-Realm', 'chinatelecom.com'))
...
def send_gprs_terminate(session_id,CCRequestNumber,Timestamp,Anumber,RAT_Type,RatingGroup,User_Equip,Cell_ID,UseUnit,CCTime):
 CCR_avps = []
 CCR_avps.append(encodeAVP('Session-ID', session_id))
 CCR_avps.append(encodeAVP('Origin-Host', 'SCP001.chinatelecom.com'))
 CCR_avps.append(encodeAVP('Origin-Realm', 'chinatelecom.com'))
...
print 'Sending DATA Usage --> Session-Name:',session_id,' # MDN:',MDNINP,' # TIME:',datetime.fromtimestamp(Timestamp),' # RG:',RGINP,' # User-Equip:',UEQUIPINP,' # Location:',CGIINP,' # UsageInput:',USU
#GPRS Initial session_id,CCRequestNumber,Timestamp,Anumber,RAT_Type,RatingGroup,User_Equip
result_code=send_gprs_init(session_id,0,Timestamp,MDNINP,'6',RGINP,UEQUIPINP,CELL_ID)
if result_code == 2001:
 ...
 result_code1=send_gprs_update(session_id,1,Timestamp,MDNINP,'6',RGINP,UEQUIPINP,CELL_ID,0,0,0)
 if result_code1 == 2001:
   #print 'Full GPRS Update Event 1','success ###',session_id, '### Result Code', result_code1
   result_code2=send_gprs_update(session_id,2,Timestamp,MDNINP,'6',RGINP,UEQUIPINP,CELL_ID,USU,USU,900)
   if result_code2 == 2001:
    #print 'Full GPRS Update Event 2','success ###',session_id, '### Result Code', result_code2
    result_code3=send_gprs_update(session_id,3,Timestamp,MDNINP,'6',RGINP,UEQUIPINP,CELL_ID,USU,USU,2100)
    if result_code3 == 2001:
      ...
      result_code4=send_gprs_terminate(session_id,3,Timestamp,MDNINP,'6',RGINP,UEQUIPINP,CELL_ID,USU,600)
