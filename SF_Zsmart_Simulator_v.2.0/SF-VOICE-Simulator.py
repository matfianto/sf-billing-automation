...
if len(sys.argv)-1 < 6:
  print 'Usage: SF-VOICE-Simulator.py <Session-Name> <MDN-A> <MDN-B> <TIME> <DURATION> <ANI>'
  print 'Input param "default" will set value Session-Name:default MDN-A:6288295291295 MDN-B:6288295291296 TIME:default-now DURATION:10 ANI:0'
  sys.exit(2)
SESSION=sys.argv[1]
A_NO=sys.argv[2]
B_NO=sys.argv[3]
...
def send_voice_init(session_id,CCRequestNumber,Timestamp,Anumber,Bnumber,RatingGroup,CCTime,IMSI,ANI):
 CCR_avps=[]
...
def send_voice_terminate(session_id,CCRequestNumber,Timestamp,Anumber,Bnumber,RatingGroup,CCTime,IMSI,ANI):
 CCR_avps = []
 CCR_avps.append(encodeAVP('Session-ID', session_id))
 CCR_avps.append(encodeAVP('Origin-Host', 'tas.ims.mnc009.mcc510.3gppnetwork.org'))
 CCR_avps.append(encodeAVP('Origin-Realm', 'ims.mnc009.mcc510.3gppnetwork.org'))
...
if DURATION.lower() == 'default':
 DURATION = 10
if ANIINP.lower() == 'default':
 ANIINP = '0'
print '-----------------------------------------------------------------------------------------------------------------------------------------------------------------'
print 'Execution at ',datetime.now()
print 'Sending VOICE Usage--> Session-Name:',session_id,' ## MDN A:',A_NO,' ## MDN B:',B_NO,' ## TIME:',datetime.fromtimestamp(Timestamp),' ## DURATION:',DURATION,' ## Acc-NW-Info(ANI):',ANIINP
#Voice Initial session_id,CCRequestNumber,Timestamp,Anumber,Bnumber,RatingGroup,CCTime,IMSI,ANI
result_code=send_voice_init(session_id,0,Timestamp,A_NO,B_NO,2,DURATION,'0',ANIINP)
if result_code == 2001:
 ...
 result_code1=send_voice_terminate(session_id,0,Timestamp,A_NO,B_NO,2,DURATION,'0',ANIINP)
